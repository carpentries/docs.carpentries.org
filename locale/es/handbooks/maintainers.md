...
- Periódicamente:
  ...
  - Incluir actualizaciones en la plantilla de la lección.
...
- Dar comentarios a los colaboradores que utilizan el modelo de The Carpentries:
  ...
  - Encuentra lo que es bueno. Sea específico en cuanto a las mejoras necesarias. Usa el lenguaje motivacional
    .
...
[La propuesta de registro anual de los mantenedores] (https://github.com/carpentries/maintainer-RFCs/issues/19) tiene
información más detallada. Si un mantenedor quiere dejar el puesto
...
- {{'[El canal #maintainers solo por invitación] ({}/archives/C8H5LN44V) '.format (slack)}} del espacio de trabajo de Slack de The Carpentries es una plataforma para que toda la comunidad haga preguntas y participe en debates sobre el tema del mantenimiento de las clases. Si eres un mantenedor y aún no tienes acceso a este canal, ponte en contacto con el líder de la comunidad de mantenedores.
...
Una vez programadas, todas las llamadas de la comunidad de mantenedores aparecen en nuestro
{{'[Community Calendar] ({}/community/events/) '.format (carpentries_website)}}. Puedes añadir eventos relevantes a tu calendario desde allí haciendo clic en el evento al que te gustaría asistir.
...
- [Maintainer Resources] (https://github.com/carpentries/maintainer-resources): un lugar para registrar los minutos de las llamadas de la comunidad de mantenedores y consejos útiles sobre las reglas de vuelo.
- [Solicitudes de comentarios de los mantenedores (RFC)] (https://github.com/carpentries/maintainer-RFCs) Solicitudes de comentarios sobre los cambios tecnológicos y otros problemas que afectan a los mantenedores de las lecciones.

### Cuadro temático

Puedes acceder a las listas de correo de The Carpentries desde
{{'[Topicbox] ({}/latest) '.format (topicbox)}}. A continuación se muestra una lista
de las personas relevantes para las actividades y programas incluidos en este manual
.
...
### Mantener la infraestructura de la lección actualizada en su repositorio

Periódicamente se publican nuevas versiones de la infraestructura de la lección para corregir errores e introducir nuevas funciones.
Todos los repositorios de lecciones de The Carpentries están configurados para recibir solicitudes de cambios automatizadas poco después del lanzamiento.
Estas solicitudes de cambios se abrirán en la cuenta `carpentries-bot`, con un título similar a «Flujos de trabajo a la versión x.y.z».
Para garantizar que el sitio de su lección se compile con la versión más reciente de Workbench, combine estas solicitudes de incorporación de cambios lo antes posible una vez que se hayan abierto.
Las lecciones basadas en los archivos fuente de R Markdown también recibirán solicitudes de extracción de la cuenta `carpentries-bot` para actualizar las versiones de los paquetes de R utilizados en la lección, tituladas «Actualizar paquetes NN», donde NN es el número de paquetes.
Al igual que las actualizaciones del flujo de trabajo, estas solicitudes de cambios se deben combinar rápidamente para garantizar que la lección se compila correctamente.

Los flujos de trabajo de GitHub Actions que crean estas solicitudes de cambios están configurados para ejecutarse una vez por semana.
En ocasiones, es posible que desee activar el proceso antes de la próxima ejecución programada, por ejemplo, si la nueva versión de los flujos de trabajo o paquetes solucionará un problema que impide la creación de la lección.
Para activar estos flujos de trabajo manualmente, visita la pestaña Acciones de tu repositorio y selecciona el flujo de trabajo de la lista de la izquierda de la pantalla: ambos flujos de trabajo incluyen «Mantener» en su nombre.

! [El menú lateral de la pestaña Acciones de un repositorio de lecciones, con anotaciones para resaltar la ubicación de los dos flujos de trabajo de mantenimiento] (../img/maintain-workflows.png)

Tras seleccionar el flujo de trabajo que quieres ejecutar, selecciona «ejecutar flujo de trabajo» cerca de la esquina superior derecha del historial de ejecuciones de flujos de trabajo. Puede dejar los valores predeterminados del pequeño menú desplegable sin cambios y volver a seleccionar «Ejecutar flujo de trabajo».

! [La interfaz para ejecutar manualmente un flujo de trabajo, anotada para resaltar la parte más relevante de la interfaz] (../img/run-workflow.png)

Si este proceso no funciona o recibes mensajes de error del flujo de trabajo y este no se completa, ponte en contacto con el [Equipo del plan de estudios] (mailto:curriculum@carpentries.org) o pide ayuda en el {{'[workbench channel en Slack] ({}/archives/C03DEQ5T2DA) '.format (slack)}}.
...
de Carpentries incluye {{'[a Help Wanted page] ({}/lessons/help_wanted) '.format (carpentries_website)}}, que puede
enumerar automáticamente todos los números etiquetados como «se busca ayuda» en los repositorios
...
Si planeas dejar temporalmente el puesto de mantenedor (
, por ejemplo, el permiso parental, la temporada de exámenes, etc.), puede ser útil que tus compañeros de la lección
y posibles colaboradores se lo hagas saber. Estas son las medidas
...
2. Informe a sus comantenedores:
   ...
   8. Si necesitas ayuda para encontrar la información de contacto de alguno de tus compañeros de mantenimiento
      , ponte en contacto con el líder de la comunidad de mantenedores.
...
para los mantenedores una vez que hayan completado la incorporación. Incluye [consejos para gestionar los problemas en un repositorio] (https://carpentries.github.io/maintainer-onboarding/02-communicate-contributors#top-ten-tips-for-managing-issues-and-prs), orientación para [etiquetar al equipo curricular en relación con los problemas y las solicitudes de cambios] (https://carpentries.github.io/maintainer-onboarding/03-communicate-maintainers.html#github) y mucha otra información útil a la que quizás quieras volver después de la incorporación.
...
### [Manual para desarrolladores de lecciones] (/handbooks/lesson_developers.md)

El manual para que los miembros de la comunidad desarrollen nuevas lecciones incluye información, orientación y otros recursos que también pueden ser interesantes para los mantenedores.
...
Sin embargo, si los cambios son pequeños y corrigen algo que, sin lugar a dudas, no funciona en la lección (por ejemplo, un enlace roto, una sintaxis mal formada o algo similar), puede combinar sus propios cambios de inmediato.
...
Aunque tienen su origen en buenas intenciones, estos comportamientos pueden tener el desafortunado efecto de disuadir a otros posibles colaboradores de trabajar en un problema o crear una solicitud de cambios equivalente.
